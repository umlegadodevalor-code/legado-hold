...
def get_usdbrl():
    ...
    if TWELVEDATA_TOKEN:
        try:
            r = _session.get(
                ...
                params={"symbol": "USD/BRL", "apikey": TWELVEDATA_TOKEN},
                timeout=12,
            )
            if r.ok:
                data = r.json()
    ...
    if BRAPI_TOKEN:
        try:
            ...
            if r.ok:
                data = r.json()
                candidates = data.get("currency") or data.get("currencies") or data.get("results") or []
...
def diagnostics():
    checks = {
        "backend": {"ok": True},
        "brapi_token": {"ok": bool(BRAPI_TOKEN)},
        "twelvedata_token": {"ok": bool(TWELVEDATA_TOKEN)},
    }

    try:
        q = get_brapi("PETR4")
        checks["brapi_petr4"] = {"ok": bool(q), "source": q.get("source") if q else None}
    except Exception as e:
        checks["brapi_petr4"] = {"ok": False, "error": str(e)}

    try:
        q = get_crypto("BTC")
        checks["crypto_btc"] = {"ok": bool(q), "source": q.get("source") if q else None}
    except Exception as e:
        checks["crypto_btc"] = {"ok": False, "error": str(e)}

    if TWELVEDATA_TOKEN:
        try:
            q = get_twelve("AAPL")
            checks["twelve_aapl"] = {"ok": bool(q), "source": q.get("source") if q else None}
        except Exception as e:
            checks["twelve_aapl"] = {"ok": False, "error": str(e)}

        try:
            fx = get_usdbrl()
            checks["usdbrl"] = {"ok": bool(fx), "value": fx}
        except Exception as e:
            checks["usdbrl"] = {"ok": False, "error": str(e)}
...
def api_quotes():
    ...
    try:
        ...
        # Câmbio só é necessário se realmente houver ativo em USD.
        precisa_cambio = any(str(a.get("type") or "").lower() == "eua" for a in assets)
        usdbrl = get_usdbrl() if precisa_cambio else None
        ...
        for asset in assets:
            ...
            try:
                ...
                if typ == "tesouro":
                    ...
                    maturity = str(asset.get("maturity") or "").strip()
                    if not family or not maturity:
                        results.append({
                            "key": key, "symbol": symbol, "ok": False,
                            "error": "Tesouro requer família e vencimento exato"
                        })
                        continue
                    quote = get_tesouro(family, maturity)
                elif typ == "eua":
                    if not TWELVEDATA_TOKEN:
                        results.append({
                            "key": key, "symbol": symbol, "ok": False,
                            "error": "TWELVEDATA_TOKEN não configurado no Render"
                        })
                        continue
                    ...
                    results.append({
                        "key": key, "symbol": symbol, "ok": False,
                        "error": "ativo configurado como manual"
                    })
                    continue
                else:
                    results.append({
                        "key": key, "symbol": symbol, "ok": False,
                        "error": f"tipo de ativo desconhecido: {typ or 'vazio'}"
                    })
                    continue
                ...
                if quote:
                    ...
                    if quote.get("currency") == "USD":
                        if usdbrl:
                            ...
                            item["warning"] = "cotação USD obtida, mas USD/BRL indisponível"
                    results.append(item)
                else:
                    results.append({
                        "key": key, "symbol": symbol, "ok": False,
                        "error": "cotação indisponível no provedor"
                    })

            except Exception as e:
                # Falha de um ativo nunca derruba os demais.
                results.append({
                    "key": key, "symbol": symbol, "ok": False,
        ...
        # Última barreira: devolve erro em JSON em vez de página 500 opaca.
        return jsonify({
            "ok": False,
            "error": f"erro interno em /api/quotes: {type(e).__name__}: {e}"
        }), 500
